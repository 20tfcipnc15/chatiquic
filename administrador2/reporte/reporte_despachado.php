...
class PDF extends FPDF
{
	...
	{	global $valor;
		...
		$this->Ln();
		$this->Cell(220,4,'CORRESPONDENCIA DESPACHADA',0,1,'C');
		$this->Cell(220,4,'REPORTE: '.$valor,0,1,'C');
		$this->Image('../../imagenes/loguito.jpeg',187,6,19,20,'JPEG');
		$this->Image('../../imagenes/escudo.jpeg',12,6,11,22,'JPEG');
	}
	function Footer()
	{	include("../../php/fecha_hora.php");
		...
		$sql="INSERT INTO reportes (id_report,fecha,fecha_sol,responsable,ip,accion) VALUES (NULL,'$valor','$fecha','$nom','$ip','Despachado')";
...
$pdf->Cell(20,7,'Nº CORR.',1,0,'C','1');
$pdf->Cell(52,7,'DIRIGIDO',1,0,'C','1');
...
$sql = "SELECT * FROM despachada WHERE fecha_hora like '%$valor%'";
$resp=mysql_query($sql);
...
{ 
	...
	{  
		...
		$hoja_ruta=$linea["hoja_ruta"];  
   		$tipo=$linea["tipo"];
		$sub_cadena = substr($referencias,0,50); 
		$sub= substr($referencias,50,48);
		$sub1= substr($referencias,100,45);	 
		$pdf->Cell(5,12,'',0,0,'L');
		$pdf->Cell(30,12,$fecha,1,0,'L');
		$pdf->Cell(25,12,$recibido_por,1,0,'L');
		$pdf->Cell(20,12,$correlativo,1,0,'C');
		$pdf->Cell(52,12,$dirigido,1,0,'L');
