                    ...
                    <table width="700" border="0" align="center" cellpadding="0" cellspacing="2" bgcolor="#74ABD3">
                      ...
                      <tr>
                        <td bgcolor="#B1CBE4"><table width="696" border="0" align="center" cellpadding="0" cellspacing="2" bordercolor="#EDF7FF">
                            <tr>
                              <td width="48" height="15" bgcolor="#4791C5" class="Estilo2">Imprimir</td>
                              <td width="48" height="15" bgcolor="#4791C5" class="Estilo2"><div align="center">Recibir</div></td>
                              <td height="15" bgcolor="#4791C5" class="Estilo2"><div align="center">Asignar</div></td>
...
{		
	...
	{	
		...
		{	
			...
			if($destino == $nombre or $destino == $unidad)
			{	
				$imprimir = 1;
			...
			/*else
			{
/*				$sql = "SELECT * FROM correspondencia WHERE rut like '$rut' and unidad like '$unidad' order by id_c DESC";
				$res = mysql_query($sql) or die ("Error de b&uacute;squeda en la BD: ". mysql_Error());
				$num = mysql_num_rows($res);
				if ($num <= 0)
				{	
					$sql = "SELECT * FROM correspondencia WHERE rut like '$rut' and destino like '$unidad' order by id_c DESC";
					$res = mysql_query($sql) or die ("Error de b&uacute;squeda en la BD: ". mysql_Error());
					$num = mysql_num_rows($res);
					if ($num > 0)
					{	
						...
						$fecha = $vec["fecha"];
						$id_c = $vec["id_c"];
						echo '<h1> RUT B '.$rut.'</h1>';
					}
				}
				////////***************************************
				$sql1 = "SELECT * FROM correspondencia WHERE rut like '%$rut%' and id_c > 696 order by id_c DESC limit 1";
				...
				while($control == 0)
				{			
					$sql = "SELECT * FROM correspondencia WHERE rut like '%$rut%' and id_c > 696 order by id_c ASC limit ".$cont;
					$res = mysql_query($sql) or die ("Error de b&uacute;squeda en la BD: ". mysql_Error());
					$num = mysql_num_rows($res);
					if($num > 0)
					{	$s=1;
						...
						if($unidad_2 == $unidad_1 and $fecha_2 == $fecha_1 and $rut_1 == $rut_2)
						{
							...
							{	
								...
								$correlativo = $vec["correlativo"];		
								$j++;
								echo "<tr onMouseOver='this.style.backgroundColor='#FF9900';this.style.cursor='hand';' onMouseOut='this.style.backgroundColor='#CC6666''o'];'>";
