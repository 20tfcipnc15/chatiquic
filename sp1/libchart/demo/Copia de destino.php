...
{	$chart = new PieChart(550,450);
	...
	$sql = "SELECT distinct(destino),COUNT(destino) as total
			...
			WHERE ".$cad." and destino !='' and fecha like '%$fecha%'
			GROUP BY destino
	...
	$chart->setDataSet($dataSet);
	$chart->setTitle("Destino de los tramites");
...
elseif($long > 5)
{
	$chart = new PieChart(550,450);
	$dataSet = new XYDataSet();
	$pdf->Cell(0,4,'FLUJO DE TRAMITES FACULTATIVO DIA: '.$fecha,0,1,'C');
	...
	$chart->setDataSet($dataSet);
	$chart->setTitle("Destino Diario de los Tramites");
