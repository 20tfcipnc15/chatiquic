...
$pdf->SetFont('Times','B',12);
$pdf->Cell(0,4,'TRAMITES PENDIENTES: '.$usuario.' - DECANATO',0,1,'C');
$pdf->SetFont('Arial','B',10);
$pdf->Ln();
$pdf->Ln();

/*m$sql7 = "SELECT id_unidad
		 FROM unidad 
...
$sql = "SELECT * FROM correspondencia WHERE sw = 1 and destino like '$nombre' GROUP BY rut order by id_c DESC";
$res = mysql_query($sql) or die ("Error de b&uacute;squeda en la BD: ".mysql_error());
$num = mysql_num_rows($res);
if($num > 0)
{			

    while($linea1=mysql_fetch_array($res))
	{  	
		$cont++;
