...
$campo[8]= 'comentario';
$sw=1;
//comprobar un campo lleno por lo menos
for ($i=0;$i<=8;$i++)
{
	$dato=$vector[$i];
	if($dato != null)
...
//si por lo menos hay un campo listar...
if ($sw == 0)
{	
...
<?php
$total = 0;
$sql = "select distinct(referencias),id_c from correspondencia where ".$cadena." GROUP BY referencias ORDER BY id_c DESC";
//$sql = "select distinct(referencias),id_c from correspondencia where ".$cadena." GROUP BY referencias ORDER BY id_c DESC";
...
if ($num > 0)
{
	while($fila = mysql_fetch_array($res))
	{
		...
		//$consulta ="SELECT * FROM correspondencia WHERE referencias = '$ref' and fecha like '%$fecha%'order by id_c ASC limit 1";
		//$consulta ="SELECT * FROM correspondencia WHERE id_c = '$ref' order by id_c ASC limit 1";
		$consulta ="SELECT * FROM correspondencia WHERE id_c = $iddd order by id_c ASC limit 1";
...
$sql="select distinct(referencias),id_c from correspondencia where ".$cadena." GROUP BY referencias ORDER BY fecha DESC";
$res=mysql_query($sql) or die ("Error de b&uacute;squeda en la BD: ". mysql_Error());
$num = mysql_num_rows($res); 
if ($num > 0) 					
{	while($fila=mysql_fetch_array($res))
