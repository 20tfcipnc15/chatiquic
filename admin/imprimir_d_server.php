...
<!--<form name="pdf" method="post" action="http://localhost:8080/prueba_pdf/SilentPrintServlet">-->
<form name="pdf" method="post" action="http://200.7.170.250:8080/prueba_pdf/SilentPrintServlet">
...
//***********************
	$id = $vector[$i];
	$sql = "SELECT * FROM correspondencia WHERE rut like '$id' and fecha like '%2015%' and destino like '$unidad' order by id_c ASC";
	...
	{	
		...
		$url2 = $fecha1.'::'.$rut.'::'.$unidad1.'::'.$correlativo1.'::'.$referencias1;
?>
<?	}
	else
	{
...
//***********************
	$sql = "SELECT * FROM correspondencia WHERE rut like '$id' and fecha like '%2015%' and unidad like '$unidad' order by id_c DESC";
	$resp = mysql_query($sql);
	$num = mysql_num_rows($resp); 
	if ($num > 0) 					
	{
		...
		if ($url2 == "")
			...
			$url = $url2.'::'.$destino.'::'.$correlativo.'::'.$comentario.'::'.$fecha;	
?>		  
