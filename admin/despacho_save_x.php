...
if ($numResultados>0)
{
	...
	//Obtenemos la parte numerica, para incrementar su valor como nuevo Tramite
	$B = '-';
...
//Insertamos en la ultima posicion el destino original
//****************************************
...
//Insertamos los registros el numero de veces que indique la longitud del vector "vector" o "vector_2"
////////*********************
/*if($long > 1)
	$comentario = 'COPIA ';
else
...
for($i=0; $i <= $long; $i++)
{
    ...
    {   
		...
		$clsInstanceName=new jUploadPHP($_FILES['fTheFileField1'],true);
		...
		if($clsInstanceName->uploadFile())
		{
			...
			$gay = (string)$clsInstanceName->getFullFileLocation();
			
		}
		//hasta aqui el jUploadphp
	...
	(NULL,'$rut1','$unidad','$fecha','$correlativo','$hoja_ruta','$tipo','$referencias','$fojas','$nombre','$destino','$comentario','$ip','$estado','1','$gay','$id_tc')";
	$result = mysql_query($sql,$link);
	
	//ahora para el pclzip
		$archive=$rut1.'.'.'zip';
		$direccion="Compress/";
		$completedirection=$direccion.$archive;
		$archivo = new PclZip($completedirection);
		/* Llamamos el metodo que creara el nuevo fichero añadiendo los ficheros especificados y separados por comas */
		$creacion = $archivo->create($gay);
	//effffffff fin
	...
	$sql1 = "SELECT id_c FROM correspondencia WHERE destino like '$destino2' order by id_c DESC limit $lo";
	$result1 = mysql_query($sql1,$link);
	$linea = mysql_fetch_array($result1);
	$id_c = $linea["id_c"]; 
...
if($sw == 1)
    {$long = 0;}
header("Location: despacho_mostrar_xx.php?id_c=$id_c&long=$long");
?>
